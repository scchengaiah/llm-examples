...
class CodeVisitor(ast.NodeVisitor):
    ...
    def get_source_from_node(self, node: ast.AST) -> str:
        ...
        """
        return "\n".join(self.source_lines[node.lineno - 1:node.end_lineno]).strip()
...
def process_project(project_root: str, output_file: str):
    """
    ...
    Args:
        ...
        custom_ignore_file (Optional[str]): Path to a custom ignore file.
    """
    ignore_spec = get_ignore_spec(project_root)
...
if __name__ == "__main__":
    repo_folder_path = os.getenv('REPO_PATH')
    output_file = Path(DATA_DIR) / "py_files_structured.jsonl"
    
    process_project(repo_folder_path, output_file)
    print(f"Parsing completed. Output written to {output_file}")