...
def get_title(markdown_text):
    class TitleResponse(BaseModel):
        title: str = Field(
            description="First level header title extracted for the provided markdown text."
        )
    ...
    response = azure_openai_model.with_structured_output(TitleResponse).invoke(
        [messages]
    )
...
def prepare_docs_from_chunks():
    ...
    for i, _ in enumerate(os.listdir(chunk_dir)):
        ...
        if os.path.exists(file_path):
            ...
            parent_docs.append(
                Document(page_content=markdown_content, metadata=parent_doc_metadata)
            )
            ...
            for doc in splits:
                ...
                for markdown_symbol, metadata_key in headers_to_split_on:
                    if metadata_key in doc.metadata:
                        header_content += (
                            f"{markdown_symbol} {doc.metadata[metadata_key]}\n"
                        )
...
def ingest_to_vectorstore_with_multi_vector_retriever_pgstore(recreate_collection=True):
    ...
    retriever.vectorstore.add_documents(
        child_docs, ids=[doc.metadata["id"] for doc in child_docs]
    )
...
def multi_vector_hybrid_retrieval(input_queries):
    ...
    for input_query in input_queries:
        ...
        results = conn.execute(
            sql, {"query": cleaned_query, "embedding": query_embeddings, "k": k}
        ).fetchall()
        ...
        for d in reranked_docs:
            if (
                multi_vector_retriever.id_key in d.metadata
                and d.metadata[multi_vector_retriever.id_key] not in ids
            ):
...
def query_expansion_with_multi_vector_hybrid_retrieval():
    ...
    response = azure_openai_model.with_structured_output(
        Questions, method="json_schema"
    ).invoke(messages)
    docs = multi_vector_hybrid_retrieval(
        input_queries=[user_query] + response.questions
    )
    return docs
...
print(
    f"\n\nTime taken to Generate complete Response: {time.time() - response_start_time} seconds"
)
