...
from parser.pdf.helpers.get_text_lines import get_raw_lines, is_white
from parser.pdf.helpers.multi_column import column_boxes
...
# Older impln to refer images in the markdown document.
GRAPHICS_TEXT = "\n![](%s)\n"
# To describe images.
GRAPHICS_TEXT = "\n%s\n"
...
def to_markdown(
    ...
    *,
    ...
    generate_img_desc_llm_callable=None,
    image_path="",
    image_format="png",
    force_text=True,
    page_chunks=False,
    ...
    Args:
        ...
        generate_image_description: (bool) whether to generate image description.
        llm_callable: callable to generate image descriptions.
    ...
    if generate_img_desc and generate_img_desc_llm_callable is None:
        raise ValueError("Generate image description requires a callable that can analyze the images and return the description of the image.")
    DPI = dpi
    ...
    def save_image(page, rect, i):
        ...
        if write_images is True:
            pixmap_obj = page.get_pixmap(clip=rect, dpi=DPI)
            pixmap_obj.save(image_filename)

            image_filename = image_filename.replace("\\", " / ")
            img_data += f"![]({image_filename})\n"
        
        if generate_img_desc is True:
            pixmap_obj = page.get_pixmap(clip=rect, dpi=DPI)
            base64_encoded_image = base64.b64encode(pixmap_obj.tobytes()).decode('utf-8')
            img_desc_from_llm = f"""\n<IMAGE_IN_TEXTUAL_REPRESENTATION>\n
            {generate_img_desc_llm_callable(base64_encoded_image)}\n
            </IMAGE_IN_TEXTUAL_REPRESENTATION>
            """
            img_data += '\n'.join(line.strip() for line in img_desc_from_llm.splitlines())
